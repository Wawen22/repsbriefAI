...
- Apify trend resilience: direzione a quattro fonti approvata (YouTube, RSS, Reddit e Google Trends); spec in `docs/superpowers/specs/2026-09-02-apify-trend-resilience-design.md`. Task 1–4 completati localmente: contratti Zod/config feature-flag, persistenza, adapter e worker asincrono. Il Task 4 aggiunge scheduling orario idempotente per fonte/nicchia, backoff 5/15/45, circuit breaker, start/poll server-side Apify e firma webhook HMAC constant-time. Gli adapter scartano record malformati, HTTP, timestamp invalidi e duplicati per fonte/id; provenance contiene esclusivamente identificatori pubblici. Reddit/Google Trends restano disabilitate per Fitness. Nessun Actor a pagamento, secret o migration è stato usato/applicato. Validazione Task 4: 8 test focalizzati verdi.
