...
- Apify trend resilience: direzione a quattro fonti approvata (YouTube, RSS, Reddit e Google Trends); spec in `docs/superpowers/specs/2026-09-02-apify-trend-resilience-design.md`. Task 1–3 completati localmente: contratti Zod/config feature-flag, migration versionata non applicata, repository server-only e adapter puri per le quattro fonti. Gli adapter scartano record malformati, HTTP, timestamp invalidi e duplicati per fonte/id; provenance contiene esclusivamente identificatori pubblici. Reddit/Google Trends restano disabilitate per Fitness. Nessun Actor a pagamento, secret o migration e stato usato/applicato. Validazione Task 3: test adapter (4), typecheck e lint mirato verdi.
