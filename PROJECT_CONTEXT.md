...
- Resend/Vercel audit: Vercel MCP confirms production `READY` on `5755e6d` with no runtime errors in the preceding 24 hours. The correct Resend sender domain `repsbrief.com` is in `pending` verification (US East): its DKIM TXT plus SPF MX/TXT records were submitted in the DNS zone and Resend verification was initiated on 2026-09-02. Wait for `verified` before configuring `RESEND_FROM_EMAIL`. The pre-existing `wai.wawen.io` sender domain is unchanged and not used by RepsBrief. The installed code fails closed for production mail when the sender is absent. The Vercel MCP surface does not expose environment-variable mutation, so that final setting remains a dashboard action.
