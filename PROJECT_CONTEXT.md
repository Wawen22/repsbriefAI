...
- Apify trend resilience: direzione a quattro fonti approvata (YouTube, RSS, Reddit e Google Trends); spec in `docs/superpowers/specs/2026-09-02-apify-trend-resilience-design.md`. Task 1 e 2 completati localmente: contratti Zod/config feature-flag, migration versionata non applicata e repository server-only per run, segnali, snapshot ed evidence. Reddit/Google Trends restano disabilitate per Fitness. Nessun Actor a pagamento, secret o migration e stato usato/applicato. Validazione Task 2: typecheck e 89 test verdi; lint completo inconclusivo per stall del runner, mentre il lint mirato dei file TypeScript e verde.
