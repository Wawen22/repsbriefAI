...
- Resend/Vercel audit: Vercel MCP confirms production `READY` on `5755e6d` with no runtime errors in the preceding 24 hours. The correct Resend sender domain `repsbrief.com` was created (US East) and is `not_started`: its DKIM TXT plus SPF MX/TXT records must be added in the `repsbrief.com` DNS zone before `RESEND_FROM_EMAIL` can be configured. The pre-existing `wai.wawen.io` sender domain is unchanged and not used by RepsBrief. The installed code fails closed for production mail when the sender is absent. The Vercel MCP surface does not expose environment-variable mutation, so that final setting remains a dashboard action.
