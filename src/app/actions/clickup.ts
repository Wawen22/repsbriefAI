...
import { getClickUpWorkspaces } from "@/lib/integrations/clickup"
...
export async function testClickUpIntegrationAction(teamId: string) {
  ...
  if (!integration?.id) return { success: false, error: "Integrazione ClickUp non trovata" }
  ...
  if (!accessToken) return { success: false, error: "Token ClickUp non disponibile, riconnetti l'account" }
  ...
  try {
    ...
    await supabase.from("team_integration_logs").insert({
      ...
      details: {
        ...
        workspace_ids: workspaces.map((workspace) => workspace.id).filter(Boolean),
      },
    })

    return { success: true, workspaceCount: workspaces.length }
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "ClickUp test failed"
...
export async function disconnectClickUpIntegrationAction(teamId: string) {
  const supabase = await createClient()
  const { data: auth } = await supabase.auth.getUser()
  if (!auth.user) return { success: false, error: "Unauthorized" }
