...
import { 
  ...
  MessageSquarePlus, ArrowRight, Share2, Download, ExternalLink, Box
} from "lucide-react"
...
export function BriefCard({ 
  ...
  const downloadMarkdown = () => {
    const content = `# ${currentIdea.title}\n\n## Hook\n> ${currentIdea.hook}\n\n## Concept\n${currentIdea.description}\n\n## AI Script\n${currentIdea.scriptDraft || 'No script generated'}\n\n## Strategy\n- **Format:** ${currentIdea.format}\n- **Why it works:** ${currentIdea.whyItWorks}\n- **Audio:** ${currentIdea.trendingAudioSuggestion || 'N/A'}`
    
    const blob = new Blob([content], { type: 'text/markdown' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `RepsBrief-${currentIdea.title.replace(/\s+/g, '-').toLowerCase()}.md`
  ...
  const CopyBtn = ({ field, text, label, icon: CustomIcon }: { field: CopyField, text: string, label: string, icon?: any }) => (
    <Button
      variant="ghost"
      size="sm"
      onClick={(e) => {
    ...
    >
      {copied === field ? (
        ...
        <>{CustomIcon ? <CustomIcon className="w-3 h-3" /> : <Copy className="w-3 h-3" />}{label}</>
      )}
    </Button>
  )
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <Button 
                    ...
                    className="h-7 text-[10px] text-slate-500 hover:text-white hover:bg-white/5 gap-1.5 font-bold uppercase tracking-wider rounded-lg"
                  >
          ...
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-10 relative z-10 custom-scrollbar">
            ...
            <div className="grid md:grid-cols-2 gap-10">
              ...
              <div className="space-y-8">
                ...
                {/* Notion/Ecosystem Sync Panel */}
                <div className="space-y-4">
                   <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-blue-600/10 flex items-center justify-center border border-blue-500/20">
                        <Box className="w-4 h-4 text-blue-400" />
                      </div>
                      <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Workspace Sync</h4>
                   </div>
                   <div className="grid grid-cols-1 gap-3">
                      <div className="group/sync flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-all">
                         <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-black flex items-center justify-center border border-white/10 font-bold text-xs">N</div>
                            <div>
                               <p className="text-xs font-bold text-white">Sync to Notion</p>
                               <p className="text-[9px] text-slate-500">Copy formatted for Notion blocks</p>
                            </div>
                         </div>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-all">
                         <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-black flex items-center justify-center border border-white/10 text-slate-400">
                               <FileText className="w-4 h-4" />
                            </div>
                            <div>
                               <p className="text-xs font-bold text-white">Export Markdown</p>
                               <p className="text-[9px] text-slate-500">Standard for Trello/ChatGPT</p>
                            </div>
                         </div>
                         <Button 
                           variant="ghost" 
                           size="sm" 
                           onClick={downloadMarkdown}
                           className="h-7 px-2 text-[10px] text-slate-500 hover:text-white hover:bg-white/5 gap-1.5 font-medium rounded-lg"
                         >
